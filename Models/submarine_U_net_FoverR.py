...
class ModelInit():  
    ...
    def Model(self):
        ...
        inFL_over_RE_beg = Input(shape=(self.params['xX'],self.params['yY'],self.params['nF']))

        ## Fluorescence Input Branch ##
        input_shape = inFL_over_RE_beg.shape
        inFL_over_RE = Conv2D(filters=self.params['nFilters2D']//2, kernel_size=self.params['kernelConv2D'], strides=self.params['strideConv2D'], 
                        padding='same', activation=self.params['activation'], input_shape=input_shape[1:], data_format="channels_last")(inFL_over_RE_beg)
        
        inFL_over_RE = Dropout(0.5)(inFL_over_RE)

        inFL_over_RE = Conv2D(filters=int(self.params['nFilters2D']/2), kernel_size=self.params['kernelConv2D'], strides=self.params['strideConv2D'], 
                        padding='same', activation=self.params['activation'], data_format="channels_last")(inFL_over_RE)
        inFL_over_RE = Dropout(0.5)(inFL_over_RE)
        ...
        #s = self.attention_gate(Up_conv_2, x, 256)
