...
class DL(Utils):    
    ...
    def Model(self):
        ...
        inRE_beg = Input(shape=(self.params['xX'],self.params['yY'],6))
        #inOP = OpData
        ...
        inRE = Conv2D(filters=self.params['nFilters2D'], kernel_size=self.params['kernelConv2D'], strides=self.params['strideConv2D'], 
                      padding='same', activation=self.params['activation'], data_format="channels_last")(inRE_beg)
        #outOP1 = inOP
        #inOP = BatchNormalization()(inOP)  
        #inOP = self.drop_out(inOP, drop_out) #drop out 1
        

        inRE = Conv2D(filters=int(self.params['nFilters2D']/2), kernel_size=self.params['kernelConv2D'], strides=self.params['strideConv2D'], 
        ...
        #inOP = BatchNormalization()(inOP)
        inRE = self.resblock_2D(int(self.params['nFilters2D']/2), self.params['kernelResBlock2D'], self.params['strideConv2D'], inRE)
        #inOP = BatchNormalization()(inOP)
