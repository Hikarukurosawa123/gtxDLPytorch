...
class Trainer(object):
    def __init__(self, config):
        ...
        self.model = FocusOnDepth(
                    image_size  =   (num_channels, resize,resize),#(config['General']['num_channel_after_concat'],resize,resize),
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for i, (X, Y_depths, Y_segmentations) in enumerate(pbar):
                ...
                output_segmentations = output_segmentations.squeeze(1) if output_segmentations != None else None
                ...
                loss = loss_fn(output_depths, Y_depths) + loss_fn(output_segmentations, Y_segmentations)
