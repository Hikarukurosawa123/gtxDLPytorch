...
from keras.layers import BatchNormalization, Input, concatenate, Conv2D, add, Conv3D, Reshape, SeparableConv2D, MaxPool2D, UpSampling2D, ZeroPadding2D, Cropping2D
...
class DL(Utils):    
    ...
    def importData(self, isTesting=True,quickTest=False):
        ...
        if getDims == 4:
            numSets = int(np.shape(self.RE)[0])
            if quickTest == False:
                while True:
    ...
    def Model(self):
        ...
        #inputDataPadded = ZeroPadding2D(padding = ((2,2), (2,2)))(inputData) # HERE
        #print("Zero padded inputData: ", inputDataPadded.shape)
        #inRE = inputDataPadded
        inRE = inputData
        ...
        print("Up_conv_1: ", Up_conv_1.shape)

        #concat_1 = concatenate([Conv_2,Up_conv_1],axis=-1)
        concat_1 = concatenate([Conv_2[:,0:Conv_2.shape[1] - 1, 0:Conv_2.shape[2] - 1, :],Up_conv_1],axis=-1) # HERE
        print("Up_conv_1: ", Up_conv_1.shape)
        ...
        Up_conv_2 = ZeroPadding2D()(Up_conv_2) # HERE
        ...
        print("Up_conv_3: ", Up_conv_3.shape)
        print("inRE: ", inRE.shape)
        Up_conv_3 = ZeroPadding2D(padding = ((1,0), (1,0)))(Up_conv_3) # HERE
        #print("Up_conv_3 zero padded: ", Up_conv_3.shape)
        
        concat_2 = concatenate([inRE,Up_conv_3],axis=-1)
        ...
        outDF = Conv2D(filters=1, kernel_size=self.params['kernelConv2D'], strides=self.params['strideConv2D'], padding='same', 
                       activation=self.params['activation'], data_format="channels_last")(outDF)
        #outDF = Cropping2D(cropping=((2,2), (2,2)))(outDF) # HERE 

        print("outDF shape", outDF.shape)
    ...
    def Fit(self,isTransfer):
        ...
        if hasattr(self, 'exportPath'):
            fileName = self.exportPath+'_trainingTime.txt'
            f = open(fileName, "a")
            f.write('Fit time = '+str(stop-start))
            f.close()
...
#                         self.ReadData() # Read in data for testing the loaded weights
    ...
    def PrintImg(self):
        ...
        numShow = int(input('Choose how many tumours you want to view: '))
        
        if img in ['RE','re']: # Print reflectance maps
            Reflectance = self.RE.copy()
            normalize = int(input('Do you want to normalize? [0/1]: '))
            if normalize:
                scale = int(input('What do you want to scale down by: '))
            ...
            for j in range(numShow):
                ...
                for i in range(fx): 
                    ...
                    axs[i].imshow(Reflectance[j, :, :, i, :])
                    axs[i].axis('off')
                    plt.colorbar(axs[i].imshow(Reflectance[j, :, :, i, :]), ax=axs[i], fraction=0.046, pad=0.04)
                plt.show()
            return None
    ...
    def Analysis(self):
        ...
        if self.DF.shape[0] < 50:
            for i in range(1): #CHANGE BACK: DF.shape[0]
                fig, axs = plt.subplots(1,3)
                plt.set_cmap('jet')
                ...
                plt.colorbar(axs[2].imshow(abs(DF_error[i,:,:]),vmin=0,vmax=5), ax=axs[2],fraction=0.046, pad=0.04)
                axs[2].axis('off')
