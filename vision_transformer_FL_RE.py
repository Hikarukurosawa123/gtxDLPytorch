...
class FeedForward(tf.keras.Model):
    ...
    def call(self, x):
        print("post feed forward shape ", self.net(x).shape)
        return self.net(x)
...
class Help(Utils):
    ...
    def importDataTest(self,isTesting=True,quickTest=False): #for training data 
        ...
        dataTemp = obj['Body'].read()
        self.dataset = h5py.File(io.BytesIO(dataTemp))
        self.FL_test = self.dataset['F']
        self.OP_test = self.dataset['OP']
        self.DF_test = self.dataset['DF']
        self.QF_test = self.dataset['QF']
        ...
        if getDims == 4:
            ...
            start = time.perf_counter()
            self.FL_test = self.FL_test[:,:100,:100,0:numSets]

            self.FL_test = np.reshape(self.FL_test[indxFx,:,:,0:numSets],(len(indxFx),self.params['xX'],self.params['yY'],numSets))
            self.RE_test = self.RE_test[:,:100,:100,0:numSets]

            self.RE_test = np.reshape(self.RE_test[indxFx,:,:,0:numSets],(len(indxFx),self.params['xX'],self.params['yY'],numSets))


            self.OP_test = self.OP_test[:,:100,:100,0:numSets]
            self.DF_test = self.DF_test[:100,:100,0:numSets]
            self.DF_test = np.reshape(self.DF_test,(self.params['xX'],self.params['yY'],numSets))
            #self.temp_DF_pre_conversion = self.temp_DF_pre_conversion[:100,:100,0:numSets]
            #self.temp_DF_pre_conversion = np.reshape(self.temp_DF_pre_conversion,(self.params['xX'],self.params['yY'],numSets))
            self.QF_test = self.QF_test[:100,:100,0:numSets]
            self.QF_test = np.reshape(self.QF_test,(self.params['xX'],self.params['yY'],numSets))

            self.RE_test = np.transpose(self.RE_test, (3,0,1,2))
            self.FL_test = np.transpose(self.FL_test, (3,0,1,2))
            self.OP_test = np.transpose(self.OP_test, (3,0,1,2))
            self.DF_test = np.transpose(self.DF_test, (2,0,1))
            #self.temp_DF_pre_conversion = np.transpose(self.temp_DF_pre_conversion, (2,0,1))
            self.QF_test = np.transpose(self.QF_test, (2,0,1))

            stop = time.perf_counter()
            print('Load time = ' + str(stop-start))

            # Apply scale
            self.OP_test[:,0,:,:] *= self.params['scaleOP0']
            self.OP_test[:,1,:,:] *= self.params['scaleOP1']
            self.DF_test *= self.params['scaleDF']
            self.QF_test *= self.params['scaleQF']
            ...
            if 'scaleFL0' in self.params:
                ...
                self.FL_test *= self.params['scaleFL']
    ...
    def call_trainer(self):
        ...
        train_images = tf.cast(self.FL / self.RE, dtype=tf.float32)
        test_images = tf.cast(self.FL_test/self.RE_test,dtype=tf.float32)

        DF_max_train = np.max(self.DF, axis = (1,2))
        DF_max_test = np.max(self.DF_test, axis = (1,2))

        QF_max_train = np.max(self.QF, axis = (1,2))
        QF_max_test = np.max(self.QF_test, axis = (1,2))

        train_DF_QF = np.concat((DF_max_train, QF_max_train), axis = 1)
        test_DF_QF = np.concat((DF_max_test, QF_max_test), axis = 1)

        train_labels = tf.cast(train_DF_QF,dtype=tf.float32)
        test_labels = tf.cast(test_DF_QF,dtype=tf.float32)
        ...
        model_config = {"image_size":101,
                        ...
                        "heads":20,
                        "mlp_dim":1000}
        tconf = TrainerConfig(max_epochs=200, batch_size=64, learning_rate=5e-4)
        ...
        def train_step(dist_inputs):
            ...
            def step_fn(inputs):
                ...
                # behavior during training versus inference (e.g. Dropout).
                    ...
                    loss = tf.math.abs(logits - label_ids)
